...
A topbar View selector lets the user swap to any other module's TUI
(PVE/Ceph/PBS) without exiting the process — the runner loop catches
the SWITCH_TO_* sentinel returned by the app.
...
from proxmox_sdk.proxmox_cli.tui.chrome import VIEW_SELECT_OPTIONS, sentinel_for_module
...
class PDMTuiApp(App[object]):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Horizontal(id="topbar"):
            ...
            yield Static(self._status_text(), id="topbar_status")
...
def run_pdm_tui_once(
    bridge: ProxmoxSDKBridge,
    *,
    mode: TuiMode,
    initial_remote: str | None = None,
) -> object:
    """Run the PDM TUI once and return the app's exit result."""
    app = PDMTuiApp(
        bridge=bridge,
...
def run_pdm_tui(
    ...
    *,
    ...
    """Run the PDM Textual TUI (no return value, kept for backwards-compat)."""
    run_pdm_tui_once(bridge=bridge, mode=mode, initial_remote=initial_remote)
